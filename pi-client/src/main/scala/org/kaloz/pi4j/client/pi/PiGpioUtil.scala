...
class PiGpioUtil extends GpioUtil {
  ...
  override def getEdgeDetection(pin: Int): Int = JGpioUtil.getEdgeDetection(pin)

  override def setDirection(pin: Int, direction: Int): Boolean = JGpioUtil.setDirection(pin, direction)
